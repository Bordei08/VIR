let htmlCode = ``;
const topCards = document.querySelector(".liked-cards");
...
function removeFromFavorites(id) {
    ...
    xhr.open('DELETE',
        `http://localhost:8021/api/like/${username}/${id}`, true);
...
async function getLikedVideos() {
  const username = readCookie('userLogin');
  httpGetAsync(`http://localhost:8021/api/like_video/${username}`, function (res) {
      if (res == 'false') {
          console.log("error");
      }
      else {
          console.log("the liked videos are")
            ...
            `
              <article class = "object_article">
                <a href = "../html/video_page.html" class = "video-link" onclick = "addIdToStorage(${res.id})" >
                  ...
                  <button style="font-size:24px" id = "favorites-btn" onClick = "removeFromFavorites(${res.id})">Remove from favorites</button>
              </article>`;
